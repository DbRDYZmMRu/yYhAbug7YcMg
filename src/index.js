...
const COLLECTIONS = {
  "frith-hilton": FRITH_HILTON_JSON,
  "dr-carl-hill": DR_CARL_HILL_JSON,
  "west-to-west": WEST_TO_WEST_JSON
};
  ...
  for (const [key, jsonUrl] of Object.entries(COLLECTIONS)) {
    try {
      ...
      const found = data.find(b =>
        b.bookTitle.toLowerCase().replace(/[^a-z0-9]/g, "-").includes(bookSlug) ||
        bookSlug.includes(b.bookTitle.toLowerCase().replace(/[^a-z0-9]/g, "-"))
      );
      if (found) {
        book = found;
        collectionKey = key;
